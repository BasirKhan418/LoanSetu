    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          designation: "",
          department: "",
          state: adminData?.isSuperAdmin ? "" : adminData?.state || "",
          district: "",
          block: "",
          tenantId: adminData?.isSuperAdmin ? "" : adminData?.tenantId || "",
          canReviewSubmissions: true,
          canApprove: true,
          isActive: true,
          isVerified: false,
        ...
        setError(data.message || "Failed to create state officer");
      }
    } catch (err) {
      setError("Error creating state officer. Please try again.");
      console.error(err);
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="flex h-full w-full flex-1 flex-col gap-4 rounded-tl-2xl border border-neutral-200 bg-white p-4 md:p-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-orange-600 to-orange-500 bg-clip-text text-transparent">
                    Add New State Officer
                  </h1>
                  <p className="text-sm text-neutral-600 mt-1">
                    Create a new state officer in the system
                  </p>
                </div>
              </div>
            </div>
          ...
          {success && (
            <div className="p-4 rounded-lg bg-green-50 border border-green-200 text-green-600">
              State officer created successfully! Redirecting...
            </div>
          )}
